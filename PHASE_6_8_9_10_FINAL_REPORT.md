...
| **8 — Transcript-based AI speaking** | **COMPLETED — implementation and live AI gateway verified** | Pronunciation, active relational lesson speaking, legacy lesson speaking, and AI Tutor microphone flows now capture browser transcripts, require non-empty transcript input, use authenticated AI endpoints, and fail visibly on unsupported browsers, permissions, no speech, network, invalid response, or unavailable AI. Fabricated scores and teacher replies were removed. The live application speech route returned HTTP 200 with a validated AI assessment using the newly supplied credential. | Physical browser microphone interaction and production-authenticated route E2E still require a logged-in production test session. |
| **9 — Structured AI generation and personalized path** | **COMPLETED — implementation, live generation, and authenticated persistence verified** | Gemini request/response validation, learner-context personalization, duplicate-safe server persistence into an existing selected unit, child-row writes, cleanup on partial failure, and a database normalized-title uniqueness guard were added. Live custom-lesson generation and roadmap generation returned HTTP 200 with validated fields. Authenticated persistence returned HTTP 201, created one lesson plus vocabulary, grammar, dialogue, quiz, and speaking rows, and the duplicate request returned HTTP 409. The temporary lesson was deleted and verified absent. The existing local curriculum engine no longer fabricates a local lesson on generation failure. | The test used a temporary service-role-backed local harness; deployment must configure the same key as a server-only secret. |
| **10 — Language preference and translation assistance** | **COMPLETED — implementation and live translation verified** | The onboarding selector remains the profile-backed native-language preference. Server requests are restricted to the existing supported language catalog. Translation is authenticated, on-demand assistance over original English, and failures show “unavailable” rather than fabricated translations. The live translation route returned HTTP 200 with a validated translated-text field. | Production-authenticated translation E2E still requires a logged-in production test session. |
...
| Live application Gemini routes with supplied credential | **Pass:** speech assessment, teacher chat, translation, custom lesson generation, and onboarding roadmap were exercised locally; each returned HTTP 200 with its validated response field after the speech fallback chain was hardened. |
| Authenticated custom-lesson persistence | **Pass:** HTTP 201; one lesson plus 1 vocabulary, 1 grammar, 2 dialogue, 1 quiz, and 1 speaking row were created in the selected existing unit; the duplicate request returned HTTP 409; cleanup verification returned an empty lesson result. |
...
The supplied Gemini credential is verified against the live Gemini API and local application gateway. The supplied Supabase secret key was verified against project `jipmxnqbndgkwnlpdrkf`; authenticated custom-lesson persistence, child-row creation, duplicate rejection, and cleanup all passed in the local server harness. For deployment, keep both credentials server-only and configure them through deployment secret settings. A real signed-in browser session is still needed for manual microphone permission and production UI E2E. These are environment/account requirements, not client-side fallback paths; the application fails closed when they are unavailable.
