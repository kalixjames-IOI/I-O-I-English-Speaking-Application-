...
| **9 — Structured AI generation and personalized path** | **COMPLETED — implementation and live generation verified; persistence E2E partial** | Gemini request/response validation, learner-context personalization, duplicate-safe server persistence into an existing selected unit, child-row writes, cleanup on partial failure, and a database normalized-title uniqueness guard were added. Live custom-lesson generation and roadmap generation both returned HTTP 200 with validated fields. The existing local curriculum engine no longer fabricates a local lesson on generation failure. | Production save E2E requires `SUPABASE_SERVICE_ROLE_KEY` on the server and an authenticated test session. The client now reports this clearly instead of silently storing local-only content. |
...
| Authenticated custom-lesson persistence attempt | **Blocked by credential validation:** the supplied service-role token was rejected by Supabase REST as `Invalid API key`; no test lesson was created and no production data was changed. |
...
The supplied Gemini credential is verified against the live Gemini API and local application gateway. The supplied service-role token was rejected by the production Supabase REST API as an invalid API key, so persistence E2E remains blocked until a current service-role secret is copied from the same Supabase project (`jipmxnqbndgkwnlpdrkf`) and configured as `SUPABASE_SERVICE_ROLE_KEY`. A valid authenticated Supabase test session is also needed to exercise protected production routes. These are environment/account requirements, not client-side fallback paths; the application now fails closed when they are unavailable.
